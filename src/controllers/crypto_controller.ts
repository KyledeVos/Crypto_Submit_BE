...
export const cryptoInitialCheckController = async (): Promise<string> => {
    ...
    if (typeof countResult === "string") {
        trackLogger({
            action: "error_file", logType: "error", callFunction: "cryptoInitialCheckController",
            message: countResult
        })
        return "failure"
    } else {
        ...
        if (countResult === 0) {
            trackLogger({
                action: "log_file", logType: "info", callFunction: "cryptoInitialCheckController -> populateInitialCryptoData",
            ...
            if (populateResult !== 'success') {
                trackLogger({
                    action: "error_file", logType: "error", callFunction: "cryptoInitialCheckController -> populateInitialCryptoData",
